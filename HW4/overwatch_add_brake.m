function dy = overwatch_add_brake(t,y)
...
% 
% for j = 1:3
%     vector_O2_A(j) = PA(j)-PO2(j);
%     vector_A_B(j) = PB(j)-PA(j);
%     vector_B_C(j) = PC(j)-PB(j);
%     vector_O7_D(j) = PD(j)-PO7(j);
%     vector_D_B(j) = PB(j)-PD(j);
% end
% 
% %% Linkage mass-center displacement
% 
% PG_r2(1) = PO2(1) + L2*cos(theta_2+phi2);
% PG_r2(2) = PO2(2) + L2*sin(theta_2+phi2);
% PG_r2(3) = 0;
% 
% PG_r3(1) = PA(1) + L3*cos(theta_3+phi3);
% PG_r3(2) = PA(2) + L3*sin(theta_3+phi3);
% PG_r3(3) = 0;
% 
% 
% PG_r4(1) = PB(1) + L4*cos(theta_4+phi4);
% PG_r4(2) = PB(2) + L4*sin(theta_4+phi4);
% PG_r4(3) = 0;
% 
% 
% PG_r5(1) = PB(1) + r4*cos(theta_4);
% PG_r5(2) = PB(2) + r4*sin(theta_4);
% PG_r5(3) = 0;
% 
% PG_r6(1) = PD(1) + L6*cos(theta_6+phi6);
% PG_r6(2) = PD(2) + L6*sin(theta_6+phi6);
% PG_r6(3) = 0;
% 
% 
% PG_r7(1) = PO7(1) + L7*cos(theta_7+phi7);
% PG_r7(2) = PO7(2) + L7*sin(theta_7+phi7);
% PG_r7(3) = 0;
...
% for j = 1:3
%     vector_O2_Gr2(j) = PG_r2(j)-PO2(1,j);
%     vector_A_Gr3(j) = PG_r3(j)-PA(j);
%     vector_B_Gr4(j) = PG_r4(j)-PB(j);
%     vector_O7_Gr7(j) = PG_r7(j)-PO7(j);
%     vector_D_Gr6(j) = PG_r6(j)-PD(j);
%     
% end
...
%  VO2 = [0 0 0];
%  VO7 = [0 0 0];
%  VA(1,:) = cross(omega_2(1,:),vector_O2_A(1,:));
%  VB(1,:) = cross(omega_3(1,:),vector_A_B(1,:)) + VA(1,:);
%  VD(1,:) = cross(omega_7(1,:),vector_O7_D(1,:));
%     
% %% Link center mass velocity
% Vr2(1,:) = cross(omega_2(1,:),vector_O2_Gr2(1,:));
% Vr3(1,:) = cross(omega_3(1,:),vector_A_Gr3(1,:))+VA(1,:);
% Vr4(1,:) = cross(omega_4(1,:),vector_B_Gr4(1,:))+VB(1,:);
% Vr6(1,:) = cross(omega_6(1,:),vector_D_Gr6(1,:))+VD(1,:);
% Vr7(1,:) = cross(omega_7(1,:),vector_O7_Gr7(1,:));
% 
...
% AA(1,:) = cross(omega_2(1,:),VA(1,:));
% AB(1,:) = AA(1,:) + cross(omega_3(1,:),cross(omega_3(1,:),vector_A_B(1,:))) + cross(alpha_3(1,:),vector_A_B(1,:));
% AC(1,:) = r5a(1,:);
% AD(1,:) = cross(omega_7(1,:),VD(1,:));
% 
% %% Linkage mass acceleration
% 
% Ar2(1,:) = cross(omega_2(1,:),Vr2(1,:));
% Ar3(1,:) = AA(1,:) + cross(omega_3(1,:),cross(omega_3(1,:),vector_A_Gr3(1,:))) + cross(alpha_3(1,:),vector_A_Gr3(1,:));
% Ar4(1,:) = AB(1,:) + cross(omega_4(1,:),cross(omega_4(1,:),vector_B_Gr4(1,:))) + cross(alpha_4(1,:),vector_B_Gr4(1,:));
% Ar6(1,:) = AD(1,:) + cross(omega_6(1,:),cross(omega_6(1,:),vector_D_Gr6(1,:))) + cross(alpha_6(1,:),vector_D_Gr6(1,:));
% Ar7(1,:) = cross(omega_7(1,:),Vr7(1,:));
...
f6x = r7*sin(theta_7)-h6*L6*sin(theta_6);
f6y = -r7*cos(theta_7)+h6*L6*cos(theta_6);

f6xp = -r7*cos(theta_7)-(h6^2)*L6*cos(theta_6)-h6p*L6*sin(theta_6);
f6yp = -r7*sin(theta_7)-(h6^2)*L6*sin(theta_6)+h6p*L6*cos(theta_6);
